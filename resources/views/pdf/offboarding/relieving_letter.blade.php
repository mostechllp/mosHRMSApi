...
<p class="body-para">
    This is to confirm that <strong>{{ $empName }}</strong> (Employee Code: <strong>{{ $empCode }}</strong>) {{ $verb }}
    <strong>{{ $companyName }}</strong>. The organisation formally relieves
    {{ $empName }} of all duties and responsibilities effective from the close of business hours on their last working day.
</p>
...
<p class="body-para">
    {{ $empName }} has completed all required offboarding formalities, handed over company assets, and fulfilled notice period obligations as applicable.
    We confirm that there are no pending dues or obligations between {{ $empName }} and {{ $companyName }}.
</p>

<p class="body-para">
    We thank {{ $empName }} for their contributions to the organisation and wish them the very best in their future endeavors.
</p>
