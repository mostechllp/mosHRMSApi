...
class PayrollController extends Controller
{
    ...
    {
        $query = Payroll::with([
            'employee.user.department:id,name',
            'employee.user.designation:id,name',
        ])
    ...
    {
        $query = Employee::select('id', 'user_id', 'employee_id', 'first_name', 'last_name')
            ->with([
                'user' => function ($q) {
                    $q->select('id', 'organization_id', 'department_id', 'designation_id', 'type')
                        ->with([
        ...
        $employees = $query->get()->map(function ($employee) {
            ...
            if (isset($data['user'])) {
                $user = $data['user'];
                $data['user_type'] = $user['type'] ?? null;
                $data['organization'] = $user['organization'] ?? null;
                $data['department'] = $user['department'] ?? null;
                $data['designation'] = $user['designation'] ?? null;
                unset($data['user']);
            }
    ...
    public function getWorkingDays(Request $request): JsonResponse
    {
        ...
        $responseData = [
            ...
            'sundays_count' => (int) count($sundays),
            'sundays' => $sundays,
            'holidays_count' => (int) count($holidays),
            'holidays' => $holidays,
    ...
    /**
     * Get approved leaves of an employee for a given month to calculate LOP.
     */
    public function getLeaves(Request $request): JsonResponse
    {
        ...
        $monthDate = Carbon::createFromFormat('Y-m', $request->query('month'));
        ...
        // -----------------------------------------------
        // 1. Working days for the month (excl. Sundays & holidays)
        // -----------------------------------------------
        ...
        while ($cursor->lte($monthDate->copy()->endOfMonth())) {
            $workingDays++;
            $cursor->addDay();
        }

        // -----------------------------------------------
        // 2. Monthly salary from salary components
        // -----------------------------------------------
        $employeeSalaryComponents = EmployeeSalaryComponent::where('employee_id', $employee->id)->get();
        $monthlySalary = $employeeSalaryComponents->sum('value');
        ...
        // -----------------------------------------------
        // 4. Calculate leave days that fall within the month
        // -----------------------------------------------
        $monthStart = Carbon::parse($startDate);
        $monthEnd = Carbon::parse($endDate);

        $totalLeaveDaysInMonth = 0;
        $leavesWithDays = $leaves->map(function ($leave) use ($monthStart, $monthEnd, &$totalLeaveDaysInMonth) {
            $leaveStart = Carbon::parse($leave->start_date)->max($monthStart);
            $leaveEnd = Carbon::parse($leave->end_date)->min($monthEnd);

            // Count only working days (Mon–Sat) within the leave range
            $daysInMonth = 0;
            $day = $leaveStart->copy();
        ...
        // -----------------------------------------------
        // 5. Total allocated leaves check
        // -----------------------------------------------
        $totalAllocated = \App\Models\LeaveAllocation::where('employee_id', $employee->id)
            ->where('year', $monthDate->year)
            ->sum('allocated_days');

        // -----------------------------------------------
        // 6. LOP Calculation: >1.5 days/month = Loss of Pay
        // -----------------------------------------------
        $lop_threshold = 1.5;
        $lop_days = max(0, $totalLeaveDaysInMonth - $lop_threshold);
        $lop_deduction = round($dailySalary * $lop_days, 2);

        return response()->json([
            'success' => true,
            'data' => [
                'leaves' => $leavesWithDays,
                'total_leave_days_in_month' => round($totalLeaveDaysInMonth, 2),
                'lop_threshold_days' => $lop_threshold,
                'lop_days' => round($lop_days, 2),
                'lop_deduction_amount' => $lop_deduction,
                'salary_info' => [
                    'monthly_salary' => round($monthlySalary, 2),
                    'working_days' => $workingDays,
                    'daily_salary' => round($dailySalary, 2),
                ],
                'total_allocated_leaves' => (float) $totalAllocated,
            ]
        ]);
    }
    ...
    public function calculateOvertime(Request $request): JsonResponse
    {
        ...
        // 1. Fetch holidays and calculate working days
        // $holidays = Holiday::whereBetween('holiday_date', [$startDate, $endDate])
        //     ->pluck('holiday_date')
        //     ->map(fn($date) => Carbon::parse($date)->toDateString())
        //     ->toArray();
        ...
        $employeeSalaryComponents = EmployeeSalaryComponent::where('employee_id', $employee->id)
            ->get();

        $monthlySalary = 0;
        if ($employeeSalaryComponents->isNotEmpty()) {
            $monthlySalary = $employeeSalaryComponents->sum('value');
        }

        // 3. Calculate Rates
        $dailySalary = $workingDays > 0 ? ($monthlySalary / $workingDays) : 0;
        $normalHourlyRate = $dailySalary / 9;
        $overtimeRatePerHour = $normalHourlyRate * 1.25;
        $overtimeRatePerMinute = $overtimeRatePerHour / 60;

        // Fetch completed attendance records
        $attendanceLogs = AttendanceLog::with('breaks')
            ->where('userid', $employee->user_id)
            ->whereBetween('log_date', [$startDate, $endDate])
            ->where('log_status', 'out')
            ->whereNotNull('punch_in')
        ...
        foreach ($attendanceLogs as $log) {
            ...
            $punchOut = Carbon::parse($log->punch_out);
            ...
            if ($log->breaks) {
                foreach ($log->breaks as $break) {
                    ...
                    } elseif ($break->start_time && $break->end_time) {
                        $totalBreakMinutes += Carbon::parse($break->start_time)->diffInMinutes(Carbon::parse($break->end_time));
                    }
                }
            }

            // Deduct excess break time if break is greater than 1 hour (60 minutes)
            if ($totalBreakMinutes > 60) {
                $excessBreak = $totalBreakMinutes - 60;
                $totalMinutes -= $excessBreak;
            }

            $standardMinutes = 9 * 60; // 9 hours (including 1 hour break)

            if ($totalMinutes > $standardMinutes) {
                if ($log->is_overtime = true) {

                    $overtimeMins = $totalMinutes - $standardMinutes;

                    $hours = floor($overtimeMins / 60);
                    $minutes = $overtimeMins % 60;

                    $overtimeAmount = $overtimeMins * $overtimeRatePerMinute;

                    $overtimeDetails[] = [
                        'date' => $log->log_date,
                        'punch_in' => $log->punch_in,
                        'punch_out' => $log->punch_out,
                        'total_working_minutes' => round($totalMinutes, 2),
                        'overtime_duration' => sprintf('%02d:%02d', $hours, $minutes),
                        'overtime_minutes' => round($overtimeMins, 2),
        ...
        $totalOvertimeRemainingMinutes = $totalOvertimeMinutes % 60;

        return response()->json([
            'success' => true,
            'data' => [
                'overtime_details' => $overtimeDetails,
                'total_overtime_minutes' => round($totalOvertimeMinutes, 2),
                'total_overtime_amount' => round($totalOvertimeAmount, 2),
                'total_overtime_formatted' => sprintf('%02d:%02d', $totalOvertimeHours, $totalOvertimeRemainingMinutes),
                'rates' => [
                    'monthly_salary' => round($monthlySalary, 2),
                    'working_days' => $workingDays,
                    'daily_salary' => round($dailySalary, 2),
                    'normal_hourly_rate' => round($normalHourlyRate, 2),
                    'overtime_hourly_rate' => round($overtimeRatePerHour, 2),
                ]
            ],
        ]);
    }
    ...
    public function submitPayroll(Request $request): JsonResponse
    {
        ...
        // Find existing payroll for employee + month + year
        $payroll = Payroll::where('user_id', $employee->user_id)
        ...
        if (!$payroll) {
            ...
            $payroll->user_id = $employee->user_id;
            $payroll->pay_period_month = (int) $request->pay_period_month;
            $payroll->pay_period_year = (int) $request->pay_period_year;
        }
        ...
        try {
            ...
            if (view()->exists('pdf.payslip')) {
                ...
                $pdf = Pdf::loadView('pdf.payslip', $this->getPayslipData($payroll));
                ...
                // Same filename means the old PDF gets replaced
    ...
    public function sendPayslip($id): JsonResponse
    {
        ...
        try {
            ...
            $payslipData = $this->getPayslipData($payroll);

            $pdf = Pdf::loadView('pdf.payslip', $payslipData)->setPaper('a4', 'portrait');
    ...
    /**
     * Show a single payroll record by ID — structured payslip response.
     */
    ...
    /**
     * Helper method to generate standard structured payslip data.
     */
    private function getPayslipData(Payroll $payroll): array
    {
        $payroll->loadMissing([
            'employee.user.department:id,name',
            'employee.user.designation:id,name',
        ...
        $employee = $payroll->employee;
        $user = $employee?->user;
        $stepData = $payroll->data ?? [];

        // ------------------------------------------------------------------
        // 1. Pay-period calendar info
        // ------------------------------------------------------------------
        $startDate = Carbon::create($payroll->pay_period_year, $payroll->pay_period_month, 1)->startOfMonth();
        $endDate = $startDate->copy()->endOfMonth();
        $totalDays = $startDate->diffInDays($endDate) + 1;

        $daysPresent = (int) (
            $stepData['step_1']['days_present'] ??
            $stepData['step_2']['days_present'] ?? 0
        );
        $workingDays = (int) (
            $stepData['step_1']['total_working_days'] ??
            $stepData['step_2']['total_working_days'] ?? $totalDays
        );
        $periodStart = $stepData['step_1']['period_start'] ?? $startDate->toDateString();
        $periodEnd = $stepData['step_1']['period_end'] ?? $endDate->toDateString();

        // ------------------------------------------------------------------
        // 2. Employee profile
        // ------------------------------------------------------------------
        $bankDetail = $employee?->bankDetails->first();
        ...
        // ------------------------------------------------------------------
        // 3. Disbursement / bank info
        // ------------------------------------------------------------------
        $ifscBranch = 'N/A';
        if ($bankDetail) {
            $parts = array_filter([$bankDetail->ifsc_code, $bankDetail->branch_name]);
            $ifscBranch = $parts ? implode(' / ', $parts) : 'N/A';
        }
        ...
        $disbursementInfo = [
            ...
            'iban_number' => $bankDetail?->iban_number ?? null,
            'swift_code' => $bankDetail?->swift_code ?? null,
            'payment_mode' => $stepData['step_1']['payment_mode'] ?? 'Bank Transfer',
        ];

        // ------------------------------------------------------------------
        // 4. Earnings — merge salary components across locations
        // ------------------------------------------------------------------
        $earningsComponents = [];
        $totalEarnings = 0.0;

        foreach ($stepData['step_2']['location_breakdown'] ?? [] as $loc) {
            $currency = $loc['currency']['code'] ?? ($payroll->currency ?? 'AED');
            foreach ($loc['salary_components'] ?? [] as $comp) {
                $amount = (float) ($comp['amount'] ?? 0);
                $found = false;
                foreach ($earningsComponents as &$existing) {
        ...
        // ------------------------------------------------------------------
        // 5. Overtime — from step_3
        // ------------------------------------------------------------------
        $overtimeAmount = 0.0;
        $overtimeDetails = [];
        $step3 = $stepData['step_3'] ?? [];
        $otEntries = $step3['overtime_details'] ?? $step3['days'] ?? [];

        foreach ($otEntries as $ot) {
            $amt = (float) ($ot['amount'] ?? 0);
            if (!isset($ot['status']) || strtolower($ot['status']) === 'approved') {
                $overtimeAmount += $amt;
                $overtimeDetails[] = [
                    'date' => $ot['date'] ?? null,
                    'overtime_duration' => $ot['overtime_duration'] ?? null,
                    'amount' => $amt,
                ];
            }
        }
        if ($overtimeAmount == 0) {
            $overtimeAmount = (float) ($step3['total_overtime_amount'] ?? $payroll->overtime ?? 0);
        }
        $totalEarnings += $overtimeAmount;

        // ------------------------------------------------------------------
        // 6. Deductions — from step_4
        // ------------------------------------------------------------------
        $deductionComponents = [];
        $totalDeductions = 0.0;
        ...
        foreach ($step4['deductions'] ?? [] as $ded) {
            ...
            $deductionComponents[] = [
                'name' => $ded['name'] ?? $ded['type'] ?? 'Deduction',
                'amount' => $amt,
                'currency' => $ded['currency'] ?? ($payroll->currency ?? 'AED'),
            ];
            $totalDeductions += $amt;
        }
        if ($totalDeductions == 0) {
            $totalDeductions = (float) ($step4['total_deductions'] ?? $payroll->deductions ?? 0);
        }

        // ------------------------------------------------------------------
        // 7. Final amounts — prefer DB columns, fallback to calculation
        // ------------------------------------------------------------------
        $grossSalary = (float) ($payroll->gross_salary ?? round(
            (float) ($stepData['step_2']['gross_salary'] ?? ($totalEarnings - $overtimeAmount)),
            2
        ));
        $netPay = (float) ($payroll->net_pay ?? round($totalEarnings - $totalDeductions, 2));

        // ------------------------------------------------------------------
        // 8. Approved leaves in this month
        // ------------------------------------------------------------------
        $leaveDetails = [];
        $totalLeaveDays = 0;
        ...
        if ($employee) {
            ...
            foreach ($leaves as $leave) {
                ...
                $effectiveStart = $leaveStart->greaterThan($startDate) ? $leaveStart : $startDate;
                $effectiveEnd = $leaveEnd->lessThan($endDate) ? $leaveEnd : $endDate;
        ...
        $paymentDate = ($payroll->status === 'completed' && $payroll->updated_at)
            ...
            : ($stepData['step_1']['payment_date'] ?? null);
        ...
        return [
            ...
            'payroll' => $payroll,
            'pay_period' => Carbon::createFromDate($payroll->pay_period_year, $payroll->pay_period_month, 1)->format('F Y'),
            ...
            'payment_mode' => $disbursementInfo['payment_mode'],
            ...
            'earnings' => [
                ...
                'overtime_amount' => round($overtimeAmount, 2),
                'overtime_detail' => $overtimeDetails,
            ...
            // ── Raw step data (for wizard re-hydration) ───────────────
            'step_data' => $stepData,
        ];
    }
    ...
    private function formatPayroll(Payroll $payroll): array
    {
        ...
        // ------------------------------------------------------------------
        // 5. Resolve department, designation, and employee meta from user
        // ------------------------------------------------------------------
        $user = $employee?->user;
        ...
        return [
            ...
            'designation' => $user?->designation ? ['id' => $user->designation->id, 'name' => $user->designation->name] : null,
            'employee_type' => $user?->type,
            'joining_date' => $employee?->joining_date,
            'month' => $payroll->pay_period_month,
            'year' => $payroll->pay_period_year,
